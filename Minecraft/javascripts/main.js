...
var formatCode = function (el) {
  ...
  for (var i = 0; i < el.val().length; i++) {
    ...
    if (!el.val()[i].match(/[\s-]/)) newVal += el.val()[i];
      ...
      var tweet = $("<li></li>")
        ...
        .html(
          '<div class="message">' +
            ...
            '</div><a class="timestamp" href="http://twitter.com/#!/notch/status/' +
            tweets[i].id_str +
            '">' +
            $.timeago(tweets[i].created_at) +
            "</a>"
        )
        .appendTo(list);
    }
...
var loadFacebook = function () {
  $("#facebook .fbarea").html(
    '<iframe src="https://www.facebook.com/plugins/likebox.php?href=http%3A%2F%2Fwww.facebook.com%2Fminecraft&amp;width=300&amp;colorscheme=dark&amp;connections=10&amp;stream=false&amp;header=false&amp;height=255" scrolling="no" frameborder="0" style="border:none; overflow:hidden; width:300px; height:255px;" allowTransparency="true"></iframe>'
  );
};
    ...
    for (i in posts) {
      ...
      if (posts[i]["regular-title"]) {
        var post = $("<li></li>")
          ...
          .html(
            '<a class="title" href="' +
              posts[i]["url-with-slug"] +
              '">' +
              posts[i]["regular-title"] +
              '</a><div class="timestamp">' +
              $.timeago(posts[i].date) +
              "</div>"
          )
          .appendTo(list);
      }
...
$(document).ready(function () {
  ...
  $("#main").addClass(
    "path_" + location.toString().replace(/[^0-9a-zA-Z]/g, "")
  );
  ...
  if (location == "/") {
    ...
    $.fn.dataTableExt.oSort["duration-asc"] = function (x, y) {
      return durToTime(x) < durToTime(y)
        ? -1
        : durToTime(x) > durToTime(y)
        ? 1
        : 0;
    };
    ...
    $(".payment-method").each(function () {
      ...
      $(method).html(
        '<input type="radio" name="method-radio" value="' + methodName + '" />'
      );
      if (methodName == "") return;
      $(method).css(
        "background-image",
        "url(/images/payment/" + methodName + ".png)"
      );
    });

    $(".payment-method").mouseup(function (e) {
      var method = e.currentTarget.id.replace("payment-method-", "");
      $($($(e.currentTarget)[0]).children("input[type=radio]")[0]).attr(
        "checked",
        true
      );
